...
export async function createInventario(payload: Partial<Inventario>): Promise<Inventario> {
...
export async function updateInventario(id: number, payload: Partial<Inventario>): Promise<Inventario | null> {
  const { tipo, peso, precio, cantidad } = payload;
  const result = await pool.query(
    `UPDATE inventario
...
export async function deleteInventario(id: number): Promise<number | null> {
